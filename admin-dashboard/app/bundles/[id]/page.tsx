...
import { Save, Search, Trash2, Cpu, CircuitBoard, HardDrive, Zap, Box, Fan, ArrowRight, AlertCircle } from 'lucide-react';
...
export default function EditBundlePage() {
    ...
    useEffect(() => {
        if (Object.keys(categorySlugMap).length > 0) {
            loadBundle();
        }
    }, [bundleId, categorySlugMap]);
    ...
    const resolveCategoryKey = (category: string): string => {
        ...
        // Reverse-map using the dynamic categorySlugMap
    ...
    const loadBundle = async () => {
        try {
            const bundle = await adminApi.getBuild(bundleId);
            setName(bundle.name);
            ...
            if (bundle.imageUrl) {
                setUploadedImage([{
                    id: 'existing',
                    filename: 'bundle-image',
                    path: bundle.imageUrl,
                    url: bundle.imageUrl,
                    mimetype: 'image/jpeg',
                    size: 0
                }]);
            ...
            if (bundle.components) {
                for (const comp of bundle.components) {
                    ...
                    if (MULTI_CATEGORIES.includes(key)) {
                        ...
                        components[key].push(comp.product);
                    } else {
                        components[key] = comp.product;
                    }
                }
            }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await adminApi.updateBuild(bundleId, {
                name,
                description,
                imageUrl,
                components,

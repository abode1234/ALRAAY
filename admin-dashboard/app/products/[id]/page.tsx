        ...
        const fetchData = async () => {
            try {
                ...
                setFormData({
                    ...
                    powerConsumption: product.powerConsumption ? product.powerConsumption.toString() : '',
                ...
                // Handle Sections
                if (product.sectionItems) {
                    setSelectedSections(product.sectionItems.map((item: any) => item.sectionId));
                }

                // Fetch brands for the product's category
                if (product.category) {
                    const brandsData = await adminApi.getBrandsByCategory(product.category);
    ...
    useEffect(() => {
        const fetchBrands = async () => {
            ...
            try {
                ...
                setBrands(data);
                // Only reset brand if it's not in the new brands list
                const brandExists = data.some((b: any) => b.name === formData.brand);
                if (!brandExists && formData.brand) {
        ...
        // Only fetch if category has changed from initial load
        if (formData.category && !fetching) {
            fetchBrands();
        }
    }, [formData.category]);
